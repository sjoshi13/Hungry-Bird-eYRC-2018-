...
class Edrone():
	...
	def __init__(self):
		...
		# [x,y,z,yaw_value]
		self.drone_position = [5.64,-1.90,55.08,.00]	

		# [x_setpoint, y_setpoint, z_setpoint, yaw_value_setpoint]
		self.setpoint =[5.68,-1.91,33.40,.00] # whycon marker at the position of the dummy given in the scene. Make the whycon marker associated with position_to_hold dummy renderable and make changes accordingly
		...
		#-----------------------Add other required variables for pid here ----------------------------------------------
                ...
                self.max_values=[1575,1575,1575,1575]
		...
		#-------------------------Add other ROS Subscribers here----------------------------------------------------
	        ...
	        self.no_of_path_points=9	#Skipping waypoints that are too close to each other#------------------------------------------------------------------------------------------------------------
                self.disarm()
        ...
        def start(self):
              while(1):
                  self.pid()
                  if abs(self.error[0])<=2 and abs(self.error[1])<=2 and abs(self.error[2])<=2 and abs(self.error[3])<=1:#checking if the error is less than 0.5
	...
	def pid(self):
         ...
         #        print('P: ',(self.Kp[i]*self.error[i]),'I',((self.error_sum[i])*self.Ki[i]),'D',(self.Kd[i]*self.error_rate[i]),'Total',i,self.out[i])
                 self.error_sum[i]=0
		...
		#self.cmd.rcYaw = 1500-self.out[3]
             ...
             #   self.yaw_err_pub.publish(self.error[3])
                ...
                self.zero.publish(0)
		print("Pitch ",self.cmd.rcPitch, " Roll ",self.cmd.rcRoll," Throttle ",self.cmd.rcThrottle)
	        self.command_pub.publish(self.cmd)
