...
class Edrone():
	...
	def __init__(self):
			...
			#after tuning and computing corresponding PID parameters, change the parameters
            		...
            		self.Kp = [80,31,65,0]#600mAh
			self.Ki = [0,0,0,0]
			self.Kd = [1250,1155,75,0]
                        '''
            		self.Kp = [45,32,50, 0]#380mAh
			self.Ki = [0,0,0,0]
			self.Kd = [850,800,90,0]
            	        
            		self.Kp = [95,38,90, 0]#800mAh
			self.Ki = [0,0,0,0]
			self.Kd = [1085,1030,110,0]
	                '''
			...
			#we are taking a path of 7 points and picking every 8th point starting from 0
			# 7 * 8 = 56; 56 + 1 =57
            		...
            		self.start_pid=-1           
            		self.skip_number = 8       # Skipping some of the Vrep points that are too close
            		self.run_to = 8         # total number of points in any path
                        f=open('hb.csv','r')
                        c=f.readlines()
			total = 0
			#s1=c[1].split(',')[1][0]
			s2=c[2].split(',')[1][0]
                       ...
                       #self.total_goals=11      #Total number of goals set according to the number of hoops and insects
	...
	'''
        def ompl_move(self):
            ...
            if self.goal_no==2 or self.goal_no==4 or self.goal_no==6 or self.goal_no==8  or self.goal_no==10 or self.goal_no==12 or self.goal_no==14 or self.goal_no==16 :
                            print("criossing")
                            self.run_to=6
                            self.skip_number=14
                            self.waypoints[5]=[self.waypoints[4][0],self.waypoints[4][1],self.waypoints[4][2]+1.8,self.waypoints[4][3]]
            else:
                           self.run_to=8
                           self.skip_number=8
            for i in range(self.run_to):       
                while(1):
                    ...
                    if (abs(self.error[0])<=.8 and abs(self.error[1])<=.8 and abs(self.error[2])<=.8 and abs(self.error[3])<=2):          
                        break
...
#----------------------------------------------------------------------------------------------------------------------
	...
	def pid(self):             
			...
			if self.error[i]>3:    #Limitng the error
				self.error[i]=3
			for temp in range(3): 
				...
				if self.error_rate==0 and i!=2:         #if errors are exactly same then whycon is hidden
						...
						self.out[i]=((self.Kp[i]*self.error[i])+(self.Kd[i]*self.error_rate[i])+((self.error_sum[i])*self.Ki[i]))/165 #The calculated value is reduced by a factor so as to 
		...
		while not rospy.is_shutdown():
                 if x==0: #executed only once at the begining of the code to arm the drone 
                   if e_drone.drone_arm==1:
                    print('arming')
                    e_drone.setpoint=[e_drone.drone_position[0],e_drone.drone_position[1],26,0]#hover at the start position
                    e_drone.disarm()
                    e_drone.arm()
                    e_drone.goal_no=0
                    e_drone.drone_arm=0
