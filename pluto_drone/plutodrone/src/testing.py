...
class Edrone():
	...
	def __init__(self):
		...
		self.setpoint =[0,0,25,.00] # whycon marker at the position of the dummy given in the scene. Make the whycon marker associated with position_to_hold dummy renderable and make changes accordingly
		...
		#after tuning and computing corresponding PID parameters, change the parameters
                ...
                self.Kp = [40,25,40, 0]#600mAh
		self.Ki = [0,0,0,0]
		self.Kd = [940,875,75,0]
                '''
                self.Kp = [35,28,40, 0]#380mAh
		self.Ki = [0,0,0,0]
		self.Kd = [750,700,80,0]
               
                self.Kp = [52,23,60, 0]#800mAh
		self.Ki = [0,0,0,0]
		self.Kd = [1010,950,110,0]
	        '''
		...
		#-------------------------Add other ROS Subscribers here----------------------------------------------------
	        ...
	        self.no_of_path_points=57
                ...
                self.skip_number = 7
                self.run_to = 9
                self.total_goals=13#2n+1
             
        def start(self):
                 # print(self.error)
                  self.pid()
                  
                  if abs(self.error[0])<=1.05 and abs(self.error[1])<=1.05 and abs(self.error[2])<=1.25 and abs(self.error[3])<=2:#checking if the error is less than 0.5     
                     #print("req")
             #        rospy.sleep(.06)
           ...
           # if self.goal_no==2 or self.goal_no==4 or self.goal_no==6 or self.goal_no==8 or self.goal_no==10 :
            ...
            for i in range(self.run_to):       
                 while(1):
                     ...
                     if (abs(self.error[0])<=1 and abs(self.error[1])<=1 and abs(self.error[2])<=1.25 and abs(self.error[3])<=2):
                         #rospy.sleep(.06)
                         print("completed ",i)
                         break
	...
	def pid(self):
              ...
              if(curr_time-self.prev_time>self.sample_time):
                ...
                for i in range(4):
                 ...
                 if self.error[i]>4:
                     self.error[i]=(self.error[i]-4)/4 + 4
		 #print("err Position",i,' : ',self.error[i])
                 ...
                 if self.error_rate==0 and i!=2 :
                   self.out[i]=((self.Kp[i]*self.error[i])+(self.Kd[i]*self.error_rate[i])+((self.error_sum[i])*self.Ki[i]))/90
                 else:   
		...
		#self.cmd.rcYaw = 1500-self.out[3]
             ...
             #   self.yaw_err_pub.publish(self.error[3])
                ...
                self.store_past[0].pop(0)
                self.store_past[0].append(self.cmd.rcRoll)
                self.store_past[1].pop(0)
                self.store_past[1].append(self.cmd.rcPitch)
		#print(self.cmd)
	        self.command_pub.publish(self.cmd)
                #print(self.setpoint)
